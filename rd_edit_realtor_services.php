...
<?
  ...
  while ($row=mysql_fetch_array($res,MYSQL_ASSOC))
  {
     if ($row['personimage']!='')
	  ...
	  echo '<td width="300">'.$company_name.'</td>';
	  echo '<td width="200"><img src="/images/'.$imagename2.'" width="50" /></td>';
      echo '<td width="530">'.$row['name'].'</td><td>'.$type[$row['typeid']].'<input type="hidden" id="active'.$row['tid2'].'" name=active['.$row['tid2'].'] value="'.$act.'"></td>';
	  //echo '<td><a class="default_link del_link" href="/rd_edit_realtor_services.php?ddel='.$row['tid'].'" onclick="return confirm(\'Are you sure you want to delete this user?\')">Delete</a></td>';
	  echo '</tr>';
	  echo '</table>';
  }
?>
