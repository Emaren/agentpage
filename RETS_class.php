...
//    $this->log_write();
  ...
  function GetObject( $Resource, $Type, $ID)
  { // RETS search method
    $data ="Type={$Type}"
              ."&Resource={$Resource}"
              ."&ID={$ID}";
    $result = $this->exec( $this->host.$this->getobject_path, &$data ,true);
    return $result;
  }



  function exec( $url, &$data = '',$img = false )
  {  //execute CURL method
    $this->exec_count++;
    ...
    if (!$img)
    {
      ...
      return substr( $response, $this->response['header_size'] );
