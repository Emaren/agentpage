...
<p style="float:right;">Last run -
<?
  $sql='select * from tb_municipality where id=1';
  $ress=mysql_query($sql);
  $row=mysql_fetch_array($ress,MYSQL_ASSOC);
  echo date('F d @ g:ia',strtotime($row['feed_dated']));
?>
</p>
</div>
...
<th><a href='<? echo $web_app_path; ?>listings.php?sort=1&dir=<? echo $sort_dir[2]?>'>MLS #&nbsp;<img src=<? echo $web_app_path.'images/'.$sort_img[1].' border=0>'; ?></a></th>
<!-- <th><a href='<? echo $web_app_path; ?>listings.php?sort=1&dir=<? echo $sort_dir[3]?>'>Postal Code&nbsp;<img src=<? echo $web_app_path.'images/'.$sort_img[1].' border=0>'; ?></a></th> -->
<th><a href='<? echo $web_app_path; ?>listings.php?sort=2&dir=<? echo $sort_dir[3]?>'>Subdivision&nbsp;<img src=<? echo $web_app_path.'images/'.$sort_img[2].' border=0>'; ?></a></th>
<th><a href='<? echo $web_app_path; ?>listings.php?sort=3&dir=<? echo $sort_dir[4]?>'>Age&nbsp;<img src=<? echo $web_app_path.'images/'.$sort_img[3].' border=0>'; ?></a></th>
<th><a href='<? echo $web_app_path; ?>listings.php?sort=4&dir=<? echo $sort_dir[5]?>'>Price&nbsp;<img src=<? echo $web_app_path.'images/'.$sort_img[4].' border=0>'; ?></a></th>
<th><a href='<? echo $web_app_path; ?>listings.php?sort=5&dir=<? echo $sort_dir[6]?>'>Municipality&nbsp;<img src=<? echo $web_app_path.'images/'.$sort_img[5].' border=0>'; ?></a></th>
</tr>

<?
   while ($row=mysql_fetch_array($res,MYSQL_ASSOC))
   {
      if (dateDiff($row['cdate'],date('Y-m-d'))<=1)
        echo '<tr class="newlist">';
      else
        echo '<tr>';
	  $sql='select * from tb_listing_images where listingid='.$row['lid'].' order by ordr asc';
	  ...
	  echo '<td>'.$row['building_no'].' '.$row['street_name'].'</td><td>'.$row['listing_no'].'</td><td>'.$row['sname'].'</td><td>'.$row['building_age'].'</td><td> $'.number_format($row['listing_price'],2,'.',',');
