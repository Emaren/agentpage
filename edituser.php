...
if ($_POST['submitedit']!='')
{
  ...
  if ($id!='')
  {
    ...
    $sql.=', brokerage_website="'.htmlentities($_POST['brokerage_website'],ENT_QUOTES).'"';
    if ($_SESSION['isadmin']!=0)
      $sql.=', add_client="'.$_POST['add_client'].'"';
	if ($_POST['realtorid']!='')
      $sql.=',mlsid="'.$_POST['realtorid'].'"';
	if ($bname!='')
  ...
  else
  {
	 ...
	 if ($_POST['email']!='' && $_POST['password']!=''  && $city_choice )
	 {
	   $sql='insert into tb_users (firstname,lastname,phone,email,password,realtor,realtorid,twitter_account,brokerage_name,brokerage_website,website,cdate';
       if ($_POST['realtorid']!='')
	     $sql.=',mlsid';
	   if ($bname!='')
	     $sql.=',brokerage_image';
       ...
       if ($_SESSION['isadmin']==1)
          $sql.=',gender';
	   $sql.=') values ("'.htmlentities($_POST['firstname'],ENT_QUOTES).'","'.htmlentities($_POST['lastname'],ENT_QUOTES);
	   ...
	   $sql.='","'.htmlentities($_POST['brokerage_name'],ENT_QUOTES).'","'.htmlentities($_POST['brokerage_website'],ENT_QUOTES).'","'.htmlentities($_POST['website'],ENT_QUOTES).'","'.date('Y-m-d').'"';
       if ($_POST['realtorid']!='')
	     $sql.=',"'.$_POST['realtorid'].'"';
	   if ($bname!='')
	     $sql.=',"'.$bname.'"';
	   ...
	   if ($res)
	   {
		 ...
		 $res=mysql_query($sql);
		 $rw1=mysql_fetch_array($res,MYSQL_ASSOC);
	     $subject='Your Personal Invitation to MyAgentNow.ca';
         $bodytxt='Hello '.$_POST['firstname'].' '.$_POST['lastname'].",<br> You are now signed up for www.MyAgentNow.ca/login.php";
...
if ($msg!=''  && $_GET['idel']=='')
  {
    $_SESSION['ok_msg']=$msg;
    session_write_close();
	if ($_SESSION['realtor']=='y' or $_SESSION['isadmin']==1)
...
<td><input name="lastname" type="text" class="rounded" value="<? echo post_or_row($use,'lastname');?>" /></td>
	...
	<?
	   ...
	   while ($row=mysql_fetch_array($res,MYSQL_ASSOC))
	   {
		 ...
		 echo "</li>\n";
	     $jscript.=' $("#img_'.$row['id'].'").simpletip({ showEffect: "none",fixed: true, position: [50,-15], content: "<div><span style=\'margin-left:5px;height:16px;font-size:12px;padding-right:5px;\'>#'.$row['id'].'</span><a href=\'/edituser.php?idel='.$row['id'].'&id='.$id.'\'><img border=\'0\' src=\'/images/trash.gif\' /></a></div>"});'."\n";
	   }
	?>
	</ul></div>
	</td>
...
<?
	  ...
	  echo '<td><input type=hidden name="gender" value="'.$use['gender'].'"></td>';
?>
</tr>
<?
...
<tr>
  ...
  <th><? if ($_SESSION['isadmin']==1) echo 'Realtor ID'; else echo '&nbsp;';?></th>
</tr>
<tr>
  <td valign="top"><input name="twitter_account" type="text" class="rounded" value="<? echo post_or_row($use,'twitter_account');?>" /></td>
  <td><? if ($_SESSION['isadmin']==1) {?><select name="realtorid">
<?
   $sql='select distinct realtorid from tb_other_realtors order by realtorid';
   $res=mysql_query($sql);
   while ($rw=mysql_fetch_array($res,MYSQL_ASSOC))
   {
     echo '<option value="'.$rw['realtorid'].'"';
	 if ($use['mlsid']==$rw['realtorid'])
	   echo ' selected ';
	 echo '>'.$rw['realtorid'].'</option>';
   }
?>
  </select>
<? } else echo '&nbsp;'; ?>
...
<tr>
  ...
  <td valign="top"><input name="brokerage_website" type="text" class="rounded" value="<? echo post_or_row($use,'brokerage_website');?>" /></td>
...
<tr>
  <td valign="top">Yes &nbsp;<input type=radio value="y" <? if ($use['add_client']=='y') echo ' checked ';?> name="add_client"> No &nbsp;<input type=radio value="n" <? if ($use['add_client']=='n') echo ' checked ';?> name="add_client"></td>
  <td>&nbsp;</td>
  <td>&nbsp;</td>
</tr>
<? } ?>
...
<?
  ...
  {
?>
<input type="submit" name="submitedit" value="Send Invitation" class="sendinvite" />
<?
  }
  else
  {
?>
<input type="submit" name="submitedit" value="Edit Client" class="sendinvite" />
<?
  }
  if ($use['realtor']=='y')
  {
?>
<input type="submit" name="importcontact" onclick='window.location="importcontact.php?id=<? echo $id;?>";return false;' value="Import Contact List" class="import" />
<?
  }
  if ($error_msg!='')
